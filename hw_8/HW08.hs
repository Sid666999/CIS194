...
transposition = Conj dir1 dir2
    where
      -- Case 1: (p -> q) -> (Not q -> Not p)
      dir1 p_imp_q = modus_tollens p_imp_q
      -- Case 2: (Not q -> Not p) -> (p -> q)
      dir2 not_q_imp_not_p p = let Conj p_not_not_p not_not_p_p = double_negation
                                   not_not_p = p_not_not_p p
                                   not_not_q = modus_tollens not_q_imp_not_p not_not_p
                               in
                                 not_not_p_p not_not_q
...
de_morgan = Conj dir1 dir2
    where
      -- Case 1: Not (p \/ q) -> (Not p /\ Not q)
      dir1 not_p_or_q = Conj (\x -> not_p_or_q (Left x)) (\y -> not_p_or_q (Right y))
                                                     
      -- Case 2: (Not p /\ Not q) -> Not (p \/ q)
      dir2 (Conj not_p not_q) = \x -> case x of
                                        Left p -> not_p p
                                        Right q -> not_q q
